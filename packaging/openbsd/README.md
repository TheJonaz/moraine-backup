...
```sh
packaging/bsd-distinfo.py --tag v0.3.1
```
...
| `make package` | `moraine-0.3.1.tgz` created — which is what validates `PLIST` |
| `make port-lib-depends-check` | clean, no output |
| `portcheck` | clean |
| `pkg_add` + `moraine --version` | installs, pulls `net/rsync` + `sysutils/rclone`, prints `moraine 0.3.1` |
...
guesswork otherwise: `PKGNAME=moraine-0.3.1`, `DISTNAME=moraine-backup-0.3.1`

...
| `make package` | `moraine-0.3.1.tgz` — the `file-check` step is what validates `PLIST` |
| `pkglint -Wall` | *Looks fine.* |
| `pkg_add` + run | installs, prints `moraine 0.3.1`, `man moraine` resolves |

`DISTNAME`, `PKGNAME` and `WRKSRC` all resolve as intended — `WRKSRC` lands on
`moraine-backup-0.3.1`, the repository name rather than `DISTNAME`, which is the

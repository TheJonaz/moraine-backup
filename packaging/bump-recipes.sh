...
while IFS= read -r f; do
    case "$f" in
        */flatpak/io.thern.moraine.metainfo.xml|*/rpm/moraine.spec|*/freebsd/Makefile|*/gentoo/*.ebuild|*/flatpak/cargo-sources.json) continue ;;
        # The BSD manifests carry per-crate checksums and per-crate versions. A
        # blanket version sed would rewrite a crate that happens to sit at the
        # old version number, and would leave every hash describing the previous
        # release. Regenerate them instead — see the closing note.
        */openbsd/distinfo|*/openbsd/crates.inc|*/netbsd/distinfo|*/netbsd/cargo-depends.mk) continue ;;
    esac
    sed -i "s/$reP/$NEW/g" "$f"
...
bump-recipes: the OpenBSD and NetBSD manifests still describe v$OLD and must be
regenerated by hand — their distinfo files carry a checksum per crate, so the
version bump above deliberately skipped them:

    packaging/bsd-distinfo.py --tag v$NEW

That reads the crate list out of the tagged tarball, so run it after the tag
exists. check-crate-lists.py covers both lists, and will fail the next bump if
this is forgotten.
EOF
